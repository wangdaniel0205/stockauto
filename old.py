...
def get_current_gain():
    ...
    investment = cpBalance.GetHeaderValue(3)
    gain = cpBalance.GetHeaderValue(4)
    printlog('평가금액: ' + str(investment))
    printlog('평가손익: ' + str(gain))
...
def buy_etf(code):
    ...
    try:
        ...
        if current_price > target_price and current_price > ma5_price \
            ...
            if cpOrder.GetDibStatus() == -1:
                symbol_list.remove(code)
                dbgout('거래 실패 -> {} {}'.format(cpOrder.GetDibStatus(), cpOrder.GetDibMsg1()))
            if ret == 4:
...
if __name__ == '__main__': 
    ...
    try:
        symbol_list = read_target_stocks()
            ...
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count:
                        buy_etf(sym)
                        time.sleep(1)
                if t_now.minute in range(0,60,10) and 0 <= t_now.second <= 5: 
                    investment, gain = get_current_gain()
                    if gain > 3500:
                        write_daily_result(investment, gain)
                        if sell_all() == True:
